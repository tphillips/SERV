...
namespace SERVDAL
{
	...
	public class MemberDAL : IMemberDAL
	{
		...
		{
			string sql = 
				"UPDATE Member set  FirstName = '" + member.FirstName + "'" +
		...
		public List<Member> List(string search)
		{
			...
			string sql = String.IsNullOrEmpty(search) ? "Select * from Member" : "Select * from Member where FirstName like " + 
				SERV.Utils.String.DBSafeString("%" + search + "%") + 
					" or LastName like " + SERV.Utils.String.DBSafeString("%" + search + "%");
		...
		{
			SERVUser ret = null;
			string sql = "select * from User u join Member m on u.MemberID = m.MemberID where m.EmailAddress = " +
			             SERV.Utils.String.DBSafeString(username) + " and m.MobileNumber = " + SERV.Utils.String.DBSafeString(passwordHash);
			DataTable t = DBHelperFactory.DBHelper().ExecuteDataTable(sql);
			if (t != null && t.Rows.Count > 0)
			{
				ret = new SERVUser();
				ret.ID = Convert.ToInt32(t.Rows[0]["UserID"]);
				ret.LastLoginDate = t.Rows[0]["LastLoginDate"] != DBNull.Value ? (DateTime?)t.Rows[0]["LastLoginDate"] : null;
				ret.MemberId = Convert.ToInt32(t.Rows[0]["MemberID"]);
			}
			return ret;
		}
